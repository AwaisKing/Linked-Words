...
public final class SearchAdapter extends RecyclerView.Adapter<ResultViewHolder> implements Filterable {
    ...
    public SearchAdapter(@Nullable final SearchHistoryTable table, @NonNull final AdapterClickListener clickListener) {
        ...
        this.filter = new Filter() {
                ...
                if (Utils.isEmpty(constraint)) key = "";
                    ...
                    for (final SearchItem item : history) {
                        ...
                        for (int i = 0; i < results.size(); i++) {
                            ...
                            if (searchItem != item && !searchItem.equals(item) && !TextUtils.equals(searchItem.getText(), item.getText())) continue;
                            containsSearch = true;
                            break;
                        }

                        if (!containsSearch && item.getText().toLowerCase().contains(key)) results.add(item);
                    }
            ...
            protected void publishResults(final CharSequence constraint, FilterResults results) {
                ...
                final List<SearchItem> dataSet;
                if (results.count > 0) dataSet = (List<SearchItem>) results.values;
                else if (Utils.isEmpty(key) && historyDatabase != null) dataSet = historyDatabase.getAllItems(null);
                else dataSet = null;
                setData(dataSet);
    ...
    public ResultViewHolder onCreateViewHolder(@NonNull final ViewGroup parent, final int viewType) {
        ...
        return new ResultViewHolder(SearchItemBinding.inflate(layoutInflater, parent, false), clickListener);
    }
    ...
    public void onBindViewHolder(@NonNull final ResultViewHolder viewHolder, final int position) {
        ...
        if (!itemTextLower.contains(key) || key.isEmpty()) itemBinding.tvText.setText(itemText);
            ...
            highlightedText.setSpan(new ForegroundColorSpan(0xA72196F3), keyIndex, keyIndex + key.length(), Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    public long getItemId(final int position) {
        if (resultList == null || resultList.isEmpty()) return super.getItemId(position);
        final SearchItem searchItem = resultList.get(position);
        return searchItem == null ? position : searchItem.hashCode();
    }

    @Override
    public void onAttachedToRecyclerView(@NonNull final RecyclerView recyclerView) {
        super.onAttachedToRecyclerView(recyclerView);
        if (layoutInflater == null) layoutInflater = LayoutInflater.from(recyclerView.getContext());
    }
}